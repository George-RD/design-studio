...
Accepted system completion conditionally loads `references/design-authority/lifecycle.md`; mutations additionally load the portable profile. This is a later acceptance stage shared by the selected lane, not a new initial route or a reason for Review/Document to load Studio. Initial branch budgets and the universal hot path remain unchanged.
